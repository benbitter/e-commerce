...
const login = async (req, res) => {
  try {
    ...
    let temp = await bcrypt.compare(req.body.password, existingUser.password);
    console.log(temp)
    if (existingUser && temp) {
      // sanitize user info
      ...
      res.cookie("token", token, {
        maxAge: parseInt(process.env.COOKIE_EXPIRATION_DAYS) * 24 * 60 * 60 * 1000,
        httpOnly: true,
        secure: process.env.PRODUCTION === "true", // only https in prod
        sameSite: process.env.PRODUCTION === "true" ? "None" : "Lax",
      });
    ...
    res
      .status(500)
      .json({ message: "Some error occurred while logging in, please try again later" });
  }
};
